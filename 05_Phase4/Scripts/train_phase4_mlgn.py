...
# ── Config ─────────────────────────────────────────────────────────────────────
WORK_DIR        = Path("/cfs/home/u037341/tese/Tese-Marco/05_Phase4/Results")
DATA_DIR        = Path("/cfs/home/u037341/tese/Tese-Marco/Dataset/XML Dataset")
PROGRESS_LOG    = WORK_DIR / "phase4_progress.txt"
...
CONTRASTIVE_LAM = 0.1          # weight of contrastive loss relative to BCE
...
log(f"Contrastive λ={CONTRASTIVE_LAM}  temperature={CONTRASTIVE_T}")

best_val_loss    = float("inf")
patience_counter = 0
history = {
    "train_loss": [], "train_bce_loss": [], "train_con_loss": [],
    "val_loss": [], "val_micro_f1": [], "val_macro_f1": []
}
MODEL_SAVE = WORK_DIR.parent / "Models" / "best_mlgn_categories.pt"
MODEL_SAVE.parent.mkdir(parents=True, exist_ok=True)

for epoch in range(NUM_EPOCHS):
    # --- Train ---
    model.train()
    total_bce, total_con, total_loss_sum = 0.0, 0.0, 0.0
    ...
    for batch in train_loader:
        ...
        loss = bce_loss + CONTRASTIVE_LAM * con_loss
    ...
    log(f"Epoch {epoch+1:2d}/{NUM_EPOCHS} | "
        f"BCE={avg_bce:.4f}  Con={avg_con:.4f} (ratio={con_ratio:.2f})  "
...
# Comparison with Phase 3 Enhanced baseline
BASELINE_MICRO = 0.6736
...
log(f"vs Phase 3 Enhanced baseline: "
...
# Formula: load phase3_enhanced_test_preds.npy + mlgn_test_preds.npy → mcnemar_exact()
log("  Reminder: save test_preds_perlabel for McNemar comparison with Phase 3 Enhanced")
np.save(WORK_DIR / "mlgn_test_preds_perlabel.npy", test_preds_perlabel)
np.save(WORK_DIR / "mlgn_test_probs.npy", test_probs_all)
np.save(WORK_DIR / "mlgn_test_labels.npy", test_labels_all)
log("  Saved: mlgn_test_preds_perlabel.npy, mlgn_test_probs.npy, mlgn_test_labels.npy")

# ── SECTION 10: Report & JSON ─────────────────────────────────────────────────
log("SECTION 10: Writing report and results JSON")

report_path = WORK_DIR / "phase4_mlgn_report.txt"
with open(report_path, "w") as f:
    f.write("=" * 80 + "\n")
    f.write("PHASE 4: MLGN — CATEGORY CLASSIFICATION REPORT\n")
    f.write("=" * 80 + "\n\n")
    ...
    f.write(f"  Contrastive lambda: {CONTRASTIVE_LAM}\n\n")
    ...
    f.write(f"  {'MLGN (global thresh)':<40} {res_global['micro_f1']:>10.4f} {res_global['macro_f1']:>10.4f} {res_global['lrap']:>10.4f} {res_global['hamming']:>10.4f}\n")
    f.write(f"  {'MLGN (per-label thresh)':<40} {res_perlabel['micro_f1']:>10.4f} {res_perlabel['macro_f1']:>10.4f} {res_perlabel['lrap']:>10.4f} {res_perlabel['hamming']:>10.4f}\n\n")
    f.write(f"DELTA vs Phase 3 Enhanced (per-label):\n")
    f.write(f"  ΔMicro-F1 = {delta_micro:+.4f}\n")
    f.write(f"  ΔMacro-F1 = {delta_macro:+.4f}\n")
    f.write(f"  ΔLRAP     = {delta_lrap:+.4f}\n\n")
    ...
    for r in per_label_rows[-10:]:
        f.write(f"  {r['label']:<40} F1={r['f1']:.4f}  support={r['support']}\n")
    f.write("\nTRAINING HISTORY:\n")
    for i, (tl, bce, con, vl, mf, maf) in enumerate(zip(
        ...
        f.write(f"  Epoch {i+1:2d}: "
                f"TrainLoss={tl:.4f} (BCE={bce:.4f} Con={con:.4f})  "
...
with open(WORK_DIR / "phase4_mlgn_results.json", "w") as f:
    json.dump(results_json, f, indent=2)

log("Phase 4 MLGN COMPLETE")
